...
$(document).ready(function() {
    ...
    $('.btn1').css('background','#007dc6'); //첫번째 불켜
    $('.btn1').css('width','100px'); // 버튼의 너비 증가
    
    $('.gallery .link1').fadeIn('slow'); //첫번째 이미지 보인다..
    $('.gallery .link1 span').delay(1500).animate({top:400, opacity:1},'slow');
    ...
    //   }
      ...
      $('.btn'+cnt).css('background','#007dc6');//자신만 불켜
      $('.btn'+cnt).css('width','100px');    

      $('.gallery li span').css('top',500).css('opacity',0);
      $('.gallery .link'+cnt).find('span').delay(1000).animate({top:400, opacity:1},'slow');
   ...
   $('.mbutton').click(function(event){  //각각의 버튼 클릭시
      ...
      $('.btn'+cnt).css('background','#007dc6');//자신 버튼만 불켜 
      $('.btn'+cnt).css('width','100px');
